      ...
      if(!isset($_GET['p_cat'])){
        if(!isset($_GET['cat_id'])){
          ...
          if (isset($_GET['user_query'])) {
            ...
            $get_product = "SELECT * FROM products WHERE product_title LIKE '%$user_query%' ORDER BY 1 DESC LIMIT $start_from, $per_page";
          } else {
            ...
            <ul class="pagination">
              <?php
                ...
                if (isset($_GET['user_query'])) {
                  ...
                  $query = "SELECT * FROM products WHERE product_title LIKE '%$user_query%' ";
                } else {
                  $query="select * from products";
                              }
...
// If no products are found, display a message and don't show pagination links
if ($total_record == 0) {
    echo "<p style='font-size: 20px; font-weight: bold; color: red;'>No products found for '$user_query'.</p>";
    echo "<p style='font-size: 18px; font-weight: bold; '><a href='shop.php'>Go back to the product list</a></p>";
} else {
